...
class StatisticsController extends Notifier<StatisticsData> {
  ...
  /// 月粒度 totals（依 period 升冪），供備份上傳（`statistics.json` 每月的
  /// playCount / listenMs，見 StatisticsBackup）。
  ...
  /// 還原雲端備份（整份覆寫本機）。
  ///
  /// [monthlyTotals] 為雲端備份檔內的月粒度 totals——月粒度以雲端為準
  /// （將來明細被截斷後，月總量只有雲端是完整的）；未提供時（例：本機
  /// 測試、備份檔缺月總量）由明細單趟重建月總量。
