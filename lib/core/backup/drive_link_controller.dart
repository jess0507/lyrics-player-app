...
/// GoogleDriveBackupService 讀它決定能否同步(回前景時推;手動連結不觸發)。
///
...
class DriveLinkController extends Notifier<DriveLinkState> {
  ...
  /// App 啟動時由 GoogleDriveBackupService 呼叫:嘗試取回 session,回傳是否可同步。
  ...
  /// Drive API 回 401 / 403(scope 被撤銷)時由 GoogleDriveBackupService 呼叫。
