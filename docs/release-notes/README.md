...
## 檔案
- [`1.0.0.md`](1.0.0.md) — 首發版本(versionCode 1)
- [`1.2.11.md`](1.2.11.md) — 線上歌詞搜尋／下載、內建清單、全螢幕播放器(自 v1.2.8 起的更新)
- [`1.2.12.md`](1.2.12.md) — 修正開啟音樂檔、推播聲音震動提醒、快進秒數與播放頁預設分頁可自訂、意見回饋與重置
...
## 版本檔格式規範
- 版本檔(`X.Y.Z.md`,不加 `v`)**只放純內容**,不加任何 Markdown 標題、說明或程式碼區塊(```),否則會被一起貼進 Play Console。
...
## 新版本流程
- 在 Claude Code 執行 `/release-notes X.Y.Z`(版本號必填,不帶就不產生;skill 定義在 `.claude/skills/release-notes/`),會依上一版 tag 以來的 git log 產生版本檔、跑格式檢查並在上方清單新增連結。
- 手動撰寫時:複製最新版本檔為 `X.Y.Z.md`,改寫各語言的「本次更新內容」,並在上方清單新增連結,最後執行 `python3 .claude/skills/release-notes/check.py docs/release-notes/X.Y.Z.md` 檢查格式。
