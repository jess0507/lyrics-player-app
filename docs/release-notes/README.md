...
## 使用方式
- **自動(預設)**:`./scripts/release.sh` 打 `vX.Y.Z` tag 時,`release.yml` 會把同名的
  `vX.Y.Z.md` 拆成各語系檔隨 AAB 一起送上 Play Console,不必手動貼。
  **檔案不存在或格式不符時 release.sh 不會打 tag、CI 也會在建置前失敗**,所以出新版前要先產生版本檔並 commit。
- **手動貼**:開啟對應版本檔案,全選複製整個檔案內容,在 Play Console 的版本資訊欄位貼上;
  `<語言碼>...</語言碼>` 會自動分配到各語系。僅上架部分語系時,刪掉不需要的區段即可。
